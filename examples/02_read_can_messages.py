...
def format_can_message(msg) -> str:
    ...
    is_fd = bool(msg.usFlags & 0x0004)
    is_xtd = bool(msg.usFlags & 0x0001)
    is_rtr = bool(msg.usFlags & 0x0002)
    is_tx_ack = bool(msg.usFlags & 0x0020)

    id_fmt = f"0x{msg.ulIdentifier:08X}" if is_xtd else f"0x{msg.ulIdentifier:03X}"
    flags_parts = []
    if is_xtd:
    ...
    data_len = msg.ucDlc
    hex_data = " ".join(f"{msg.abData[i]:02X}" for i in range(min(data_len, 64)))

    return (
        f"[{msg.dHardwareTime:12.6f}s] "
        f"ID={id_fmt}  DLC={msg.ucDlc:<2d}  "
        f"Data=[{hex_data}]  "
        f"({flags_str})"
    )
...
def main():
    ...
    try:
        ...
        try:
            ...
            print(f"[OK] Baud rate set to {args.baudrate} bit/s.")
        except Exception as e:
            print(f"[WARN] Could not set baud rate ({e}). Using hardware default.")
